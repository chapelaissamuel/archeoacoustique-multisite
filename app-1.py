...
class NeuralSim:
    ...
    def sr_curve(self, E_uv, f_hz, noises):
        snrs = []
        for n in noises:
            _, h = self.kuramoto(f_hz, E_uv, n, T=0.8)
            p = self.plv(h, f_hz)
...
def run_simulation(params: dict, neural: NeuralSim, algo: IcAlgorithm):
    ...
    t, theta = neural.kuramoto(freq, E_cortex_uvm, noise, T=1.0)
...
def main():
    ...
    with st.sidebar:
        ...
        <div style="padding:0 0.2rem 1.5rem;">
            ...
            <div style="font-size:0.72rem;color:#4a5568;margin-top:0.15rem;
                ...
                v2.0 · Ic v2.1 · N=1000 · MAICR · HF Free Tier
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    if "Simulation par Site" in page:
        ...
        # ── Simulation ──
        st.markdown(section_header("3", "Paramètres & Simulation"), unsafe_allow_html=True)

        if site.chamber_resonance:
            default_freq = site.chamber_resonance
        elif site.average_frequency:
            default_freq = site.average_frequency
        else:
            default_freq = 110.0
        odf_default = site.odf if site.odf else 3.5

        col1, col2 = st.columns(2)
        with col1:
            freq = st.slider("Fréquence (Hz)", 20.0, 150.0, default_freq, 0.5,
                             key=f"freq_{site.id}")
            odf = st.slider("ODF (Texture)", 1.0, 5.0, odf_default, 0.1,
                            key=f"odf_{site.id}")
        with col2:
            noise = st.slider("Bruit synaptique (mV)", 0.1, 3.0, 1.5, 0.1,
                              key=f"noise_{site.id}")
            press_db = st.slider("Pression (dBSPL)", 80, 120, 102, 1,
                                 key=f"press_{site.id}")

        Q = st.slider("Facteur Q", 20, 1000, 500, 10, key=f"Q_{site.id}")
            ...
            if st.session_state.get('last_site_id') != site.id:
                ...
                with c1:
                    st.markdown(phys_card(
                        ...
                        "Induction B", f"{res['B_gran']*1e9:.2f}", "nT",
