...
# ============================================================================
WORLD_PRESETS = {
    "🏔️ Barabar · Chambre Sudama (Inde)": {
        "freq": 34.4, "odf": 4.2, "press_db": 102, "Q": 950,
        "material_type": "granite", "noise": 1.5,
        "note": "Mesures BAM 2023-2024. Battement 41.1 Hz @ 75.5 Hz."
    },
    "🇲🇹 Ħal Saflieni · Oracle Room (Malte)": {
        "freq": 114.0, "odf": 2.5, "press_db": 95, "Q": 600,
        "material_type": "limestone", "noise": 1.5,
        "note": "Double résonance 70+114 Hz. Debertolis 2014. EEG Cook 2008."
    },
    "🇪🇬 Grande Pyramide · Chambre du Roi (Égypte)": {
        "freq": 117.0, "odf": 3.8, "press_db": 100, "Q": 800,
        "material_type": "granite", "noise": 1.5,
        "note": "Danley 1997. Sarcophage 114-122 Hz. Infrasound 16 Hz shafts."
    },
    "🇮🇪 Newgrange · Chambre intérieure (Irlande)": {
        "freq": 110.0, "odf": 2.8, "press_db": 90, "Q": 400,
        "material_type": "mixed", "noise": 1.5,
        "note": "Jahn 1996 JASA. Princeton PEAR Lab. Pic ~110 Hz."
    },
    "🇫🇷 Lascaux · Salle des Taureaux (France)": {
        "freq": 88.0, "odf": 1.2, "press_db": 85, "Q": 200,
        "material_type": "limestone", "noise": 1.5,
        "note": "Reznikoff & Dauvois 1988. Corrélation art rupestre-résonance."
    },
    "🇲🇽 Chichen Itza · El Castillo (Mexique)": {
        "freq": 100.0, "odf": 1.5, "press_db": 90, "Q": 300,
        "material_type": "limestone", "noise": 1.5,
        "note": "Écho du serpent ~100 Hz. Calcaire yucatèque."
    },
    "🏴󠁧󠁢󠁥󠁮󠁧󠁿 Stonehenge (Angleterre)": {
        "freq": 95.0, "odf": 2.2, "press_db": 88, "Q": 180,
        "material_type": "sandstone", "noise": 1.5,
        "note": "Cox, Fazenda & Greaney 2020. Maquette 1:12 JASA."
    },
    "🇵🇪 Chavín de Huántar (Pérou)": {
        "freq": 120.0, "odf": 2.0, "press_db": 95, "Q": 350,
        "material_type": "mixed", "noise": 1.5,
        "note": "Galeries souterraines. Effets acoustiques intentionnels."
    },
    "🌍 Site personnalisé": {
        "freq": 40.0, "odf": 3.0, "press_db": 95, "Q": 400,
        "material_type": "granite", "noise": 1.5,
        "note": "Entrez vos propres paramètres ci-dessous."
    },
}
...
def validate_inputs(freq, press_db, odf, Q, noise):
    errors = []
    if not (20.0 <= freq <= 200.0):      errors.append(f"FREQ {freq:.1f} Hz hors plage [20–200 Hz]")
...
def main():
    ...
    with st.sidebar:
        ...
        <div style="padding:0 0.2rem 1.5rem;">
            ...
            <div style="font-size:0.72rem;color:#4a5568;margin-top:0.15rem;
                        font-family:'JetBrains Mono',monospace;">
                v2.0 · Ic v2.1 · MAICR 5 Agents · Whitelist 20 Refs
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        st.markdown('<div style="font-size:0.68rem;font-weight:700;text-transform:uppercase;'
                    'letter-spacing:0.14em;color:#4a5568;margin:0 0 0.5rem;">Navigation</div>',
                    unsafe_allow_html=True)
        page = st.radio("", ["🏛️ Simulation par Site", "🔬 Comparaison Multi-Sites", "📚 Documentation"],
                        label_visibility="collapsed")

        st.markdown("---")
        st.markdown('<div style="font-size:0.68rem;font-weight:700;text-transform:uppercase;'
                    'letter-spacing:0.14em;color:#4a5568;margin:0 0 0.5rem;">Clé API OpenRouter</div>',
                    unsafe_allow_html=True)
        api_key = st.text_input("", placeholder="sk-or-v1-...", type="password",
                                label_visibility="collapsed",
                                help="Clé OpenRouter pour activer le pipeline MAICR 5 agents")
        ...
        <div style="padding:1rem;background:rgba(255,255,255,0.02);
            ...
            <div style="font-size:0.68rem;color:#3a5a80;font-weight:600;
                ...
                Chaîne causale Ic v2.1
            </div>
            <div style="font-size:0.75rem;color:#5a7590;line-height:2.0;">
                Acoustique<br>
                <span style="color:#1e3a5a;margin-left:0.3rem;">↓</span><br>
                Piézoélectrique (T·G)<br>
                <span style="color:#1e3a5a;margin-left:0.3rem;">↓</span><br>
                Transmission osseuse<br>
                <span style="color:#1e3a5a;margin-left:0.3rem;">↓</span><br>
                Kuramoto N=3000 · PLV<br>
                <span style="color:#1e3a5a;margin-left:0.3rem;">↓</span><br>
                PAC Huang 2026 (C·PAC)<br>
                <span style="color:#1e3a5a;margin-left:0.3rem;">↓</span><br>
                Dual Pathway → Ic v2.1<br>
                <span style="color:#1e3a5a;margin-left:0.3rem;">↓</span><br>
                MAICR 5 Agents
            </div>
        </div>
    ...
    if "Simulation par Site" in page:
        ...
        # ── Sélecteur de site ──
        st.markdown(section_header("1", "Sélection du Site Archéologique"), unsafe_allow_html=True)

        site_labels_map = {}
        for sid, s in SITES_DB.items():
            if s.chamber_resonance:
                freq_str = f"{s.chamber_resonance:.1f} Hz"
            elif s.average_frequency:
                freq_str = f"{s.average_frequency:.0f} Hz"
            else:
                freq_str = "N/A"
            site_labels_map[f"{s.name} · {freq_str}"] = sid

        label_list = list(site_labels_map.keys())
        current_label = label_list[list(site_labels_map.values()).index(st.session_state.selected_site)]
        ...
        new_site_id = site_labels_map[selected]

        if new_site_id != st.session_state.selected_site:
            st.session_state.selected_site = new_site_id
        ...
        # ── Données documentées ──
        # BUG EMBED IFRAME : st.markdown ne se met pas à jour en mode embed.
        # Solution : st.components.v1.html() crée un mini-iframe interne qui
        # est TOUJOURS reconstruit, bypassing le diff DOM de Streamlit.
        ...
        if site.chamber_resonance:
            _, source = site.get_resonance_badge()
            data_rows_html += f'<div class="row"><span class="label">Chambre de résonance</span><span class="val">{site.chamber_resonance:.1f} Hz <span class="badge">Mesuré</span></span></div><div class="src">↳ {source}</div>'
        if site.sarcophagus_resonance:
            source = site._get_source_text("sarcophagus_resonance")
            data_rows_html += f'<div class="row"><span class="label">Résonance sarcophage</span><span class="val">{site.sarcophagus_resonance:.1f} Hz <span class="badge">Mesuré</span></span></div><div class="src">↳ {source}</div>'
        if site.documented_frequency:
            _, source = site.get_frequency_badge()
            data_rows_html += f'<div class="row"><span class="label">Fréquence documentée</span><span class="val">{site.documented_frequency:.1f} Hz <span class="badge">Documenté</span></span></div><div class="src">↳ {source}</div>'
        if site.infrasound:
            source = site._get_source_text("infrasound")
            data_rows_html += f'<div class="row"><span class="label">Infrasons</span><span class="val">{site.infrasound:.1f} Hz <span class="badge">Mesuré</span></span></div><div class="src">↳ {source}</div>'
        if site.frequency_range:
            _, source = site.get_frequency_badge()
            data_rows_html += f'<div class="row"><span class="label">Plage de résonance</span><span class="val">{site.frequency_range[0]:.0f} – {site.frequency_range[1]:.0f} Hz <span class="badge">Plage mesurée</span></span></div><div class="src">↳ {source}</div>'
        if site.beat_frequency:
            data_rows_html += f'<div class="row"><span class="label">Battement généré</span><span class="val">{site.beat_frequency:.1f} Hz <span class="badge">Calculé</span></span></div><div class="src">↳ Calculé à partir des données mesurées</div>'
        if site.material:
            odf_str = f" · ODF = {site.odf}" if site.odf else ""
            data_rows_html += f'<div class="row"><span class="label">Matériau</span><span class="val">{site.material}{odf_str} <span class="badge">Documenté</span></span></div><div class="src">↳ Analyse géologique</div>'

        section2_component = f"""
        <html><head><style>
            * {{ margin:0; padding:0; box-sizing:border-box; }}
            body {{ font-family:'Inter',system-ui,sans-serif; background:#070b14; color:#dde3f0; padding:0.5rem 0; }}
            .header {{ display:flex; align-items:center; gap:0.75rem; margin-bottom:1rem; }}
            .num {{ width:30px; height:30px; background:linear-gradient(135deg,#3a7ef4,#6b4ef7);
                     border-radius:8px; display:flex; align-items:center; justify-content:center;
                     font-size:0.8rem; font-weight:700; color:#fff; flex-shrink:0; }}
            .title {{ font-size:1.05rem; font-weight:600; letter-spacing:-0.01em; }}
            .subtitle {{ font-size:0.8rem; color:#8da0c0; margin-left:auto; }}
            .desc {{ background:rgba(58,126,244,0.06); border:1px solid rgba(58,126,244,0.15);
                      border-radius:12px; padding:1rem 1.2rem; margin-bottom:1rem;
                      font-size:0.83rem; color:#8da0c0; line-height:1.6; }}
            .data-title {{ font-size:0.92rem; font-weight:600; margin-bottom:0.6rem; }}
            .row {{ display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap;
                     gap:0.4rem; padding:0.5rem 0; border-bottom:1px solid rgba(255,255,255,0.05); }}
            .label {{ font-size:0.82rem; color:#8da0c0; }}
            .val {{ font-family:'JetBrains Mono',monospace; font-weight:600; font-size:0.9rem; color:#c8d4e8; }}
            .badge {{ display:inline-block; padding:0.1rem 0.4rem; border-radius:8px; font-size:0.65rem;
                       font-weight:600; background:rgba(0,212,138,0.15); color:#00d48a;
                       border:1px solid rgba(0,212,138,0.3); margin-left:0.3rem; }}
            .src {{ font-size:0.72rem; color:#4a6a90; padding:0.2rem 0 0.4rem; }}
        </style></head><body>
            <div class="header">
                <div class="num">2</div>
                <span class="title">{site.name}</span>
                <span class="subtitle">{site.location}</span>
            </div>
            <div class="desc">{site.description}</div>
            <div class="data-title">📊 Données Acoustiques Documentées — {site.name}</div>
            {data_rows_html}
        </body></html>
        """
        # Hauteur dynamique : base pour header + description + marge, puis par ligne de données
        n_rows = sum([
            bool(site.chamber_resonance), bool(site.sarcophagus_resonance),
            bool(site.documented_frequency), bool(site.infrasound),
            bool(site.frequency_range), bool(site.beat_frequency), bool(site.material)
        ])
        # Description longue = plus de wrapping sur mobile → plus de hauteur
        desc_chars = len(site.description)
        desc_height = max(120, int(desc_chars * 0.55))  # ~0.55px par caractère sur mobile
        component_height = 80 + desc_height + 40 + n_rows * 70
        components.html(section2_component, height=component_height, scrolling=False)
        ...
        selected_preset = st.selectbox(
            "🌍 Preset site mondial (optionnel)",
            preset_names,
            index=default_preset_idx,
            key=f"preset_{site.id}",
            help="Charge automatiquement les paramètres documentés du site. Modifiables après."
        )
        ...
        with col1:
            freq = st.slider("Fréquence (Hz)", 20.0, 200.0, float(default_freq), 0.5,
            ...
            odf = st.slider("ODF — Orientation cristalline", 1.0, 5.0, float(odf_default), 0.1,
                            key=f"odf_{site.id}", help="1=aléatoire (piézo nul) · 5=parfait · Barabar≈4.2")
        with col2:
            noise = st.slider("Bruit synaptique (mV)", 0.1, 3.0, float(default_noise), 0.1,
                              key=f"noise_{site.id}", help="Bruit Kuramoto optimal ≈ 0.10 mV")
            press_db = st.slider("Pression (dBSPL)", 60, 130, int(default_press), 1,
                                 key=f"press_{site.id}",
                                 help="102 dBSPL = voix forte Barabar · 85 = voix normale")

        Q = st.slider("Facteur Q", 20, 1000, int(default_Q), 10, key=f"Q_{site.id}",
